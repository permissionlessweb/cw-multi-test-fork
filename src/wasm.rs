...
use crate::error::{
    duplicated_code_id, duplicated_contract_address, empty_attribute_key, event_type_too_short,
    invalid_code_id, no_more_code_id_available, reserved_attribute_key, std_error_bail,
    unregistered_code_id, unsupported_wasm_query, Error,
};
...
use cosmwasm_std::{
    ...
    MigrateInfo, Order, Querier, QuerierWrapper, Record, Reply, ReplyOn, Response, StdError,
    ...
    QueryC: CustomQuery + DeserializeOwned + 'static,
{
    ...
    fn duplicate_code(&mut self, code_id: u64) -> StdResult<u64> {
        ...
        let new_code_id = self
            .next_local_code_id()
            .ok_or(StdError::msg(no_more_code_id_available()))?;
    ...
    QueryC: CustomQuery + DeserializeOwned + 'static,
{
    ...
    fn fork_state(
        ...
        env: &Env,
    ) -> StdResult<ForkState<ExecC, QueryC>> {
        Ok(ForkState {
    ...
    pub fn contract_code(&self, code_id: u64) -> StdResult<ContractBox<ExecC, QueryC>> {
    ...
    fn update_admin(
        ...
        if data.admin != Some(sender) {
            std_error_bail!("Only admin can update the contract admin: {:?}", data.admin);
        }
        // update admin field
        data.admin = admin;
    ...
    fn execute_wasm(
        ...
        match msg {
            ...
            msg => std_error_bail!(Error::UnsupportedWasmMsg(msg)),
        }
    }
    ...
    fn process_response(
        ...
        let data = messages.into_iter().try_fold(data, |data, resend| {
            ...
            Ok::<_, StdError>(sub_res.data.or(data))
        })?;
    ...
    where
        F: FnOnce(ContractBox<ExecC, QueryC>, Deps<QueryC>, Env) -> StdResult<T>,
    {
    ...
    where
        F: FnOnce(ContractBox<ExecC, QueryC>, DepsMut<QueryC>, Env) -> StdResult<T>,
...
mod test {
    ...
    fn register_contract() {
        ...
        assert!(err
            .to_string()
            .contains(&StdError::msg("Init failed").to_string()));
