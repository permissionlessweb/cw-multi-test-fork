...
use cosmwasm_std::{
    ...
    Decimal256, Delegation, DelegationResponse, DistributionMsg, Empty, Event, FullDelegation,
    Querier, StakingMsg, StakingQuery, StdResult, Storage, Timestamp, Uint256, Validator,
    ValidatorResponse,
};
...
impl StakeKeeper {
    ...
    fn get_staking_info(staking_storage: &dyn Storage) -> StdResult<StakingInfo> {
    ...
    fn get_rewards_internal(
        ...
        let delegator_rewards =
            shares.rewards + shares.share_of_rewards(validator_info, new_validator_rewards.into());

        Ok(Coin {
            denom: staking_info.bonded_denom,
            amount: Uint256::new(1).mul_floor(delegator_rewards), // multiplying by 1 to convert Decimal to Uint256
        })
    }
    ...
    fn update_rewards(
        ...
        let mut validator_info = VALIDATOR_INFO
            ...
            .ok_or_else(|| std_error!("validator does not exist"))?;
    ...
    fn get_validators(&self, staking_storage: &dyn Storage) -> StdResult<Vec<Validator>> {
    ...
    fn get_stake(
        ...
        Ok(shares.map(|shares| {
            Coin {
                denom: staking_info.bonded_denom,
                amount: Uint256::new(1).mul_floor(shares.stake), // multiplying by 1 to convert Decimal to Uint256
            }
        }))
    }
    ...
    fn update_stake(
        ...
        let mut shares = if sub {
            ...
            shares.ok_or_else(|| std_error!("no delegation for (address, validator) tuple"))?
        } else {
        ...
        if sub {
            ...
            if shares.stake < amount_dec.into() {
                std_error_bail!("invalid shares amount");
            }
            shares.stake -= amount_dec;
            validator_info.stake = validator_info.stake.checked_sub(amount.into())?;
        } else {
            shares.stake += amount_dec;
            validator_info.stake = validator_info.stake.checked_add(amount.into())?;
        }
    ...
    fn validate_denom(&self, staking_storage: &dyn Storage, amount: &Coin) -> StdResult<()> {
        let staking_info = Self::get_staking_info(staking_storage)?;
        ensure_eq!(
            amount.denom,
    ...
    // Asserts that the given coin has the proper denominator
    fn validate_percentage(&self, percentage: Decimal) -> StdResult<()> {
        ensure!(
            percentage <= Decimal::one(),
            std_error!("expected percentage")
        );
        Ok(())
    }
...
impl DistributionKeeper {
    ...
    pub fn remove_rewards(
        ...
        let rewards = Uint256::new(1).mul_floor(shares.rewards); // convert to Uint256
    ...
    pub fn get_withdraw_address(storage: &dyn Storage, delegator: &Addr) -> StdResult<Addr> {
...
impl Module for DistributionKeeper {
    ...
    fn execute<ExecC, QueryC: CustomQuery>(
        ...
        match msg {
            ...
            m => std_error_bail!("Unsupported distribution message: {:?}", m),
        }
    }
    ...
    fn query(
        ...
        std_error_bail!("Something went wrong - Distribution doesn't have query messages")
    }
...
mod test {
    use super::*;
    use crate::{
        wasm_emulation::channel::RemoteChannel, BankKeeper, FailingModule, GovFailingModule,
    ...
    use cw_orch::mock::cw_multi_test::IntoBech32;
    ...
    fn cannot_steal() {
        ...
        .unwrap_err();
        assert!(error_result.to_string().contains("invalid shares amount"));
    ...
    fn denom_validation() {
        ...
        assert!(error_result
            .to_string()
            .contains("cannot delegate coins of denominator FAKE, only of TOKEN"));
    }
    ...
    fn cannot_slash_nonexistent() {
        ...
        assert!(error_result
            .to_string()
            .contains("validator does not exist"));
    }
    ...
    fn zero_staking_forbidden() {
        ...
        assert!(error_result
            .to_string()
            .contains("invalid delegation amount"));
